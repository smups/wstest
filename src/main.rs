...
fn main() {
  ...
  watershed(&root.join("LS_00041_allPPVres1.5.fits"), &root.join("PPV/"));

  watershed(&root.join("LS_00041_dens.fits"), &root.join("PPP/"));
}
...
fn watershed(input_file: &std::path::Path, output_folder: &std::path::Path) {
  ...
  //Run pre-processor
  let cube = rws::TransformBuilder::new_merging().build().unwrap().pre_processor(data.view());
  drop(data); //dealloc old cube

  //Check that output folder exists
  if !output_folder.exists() {
    std::fs::create_dir(output_folder)
      .expect(&format!("could not create output folder {output_folder:?}"))
  }
  ...
  cube.axis_iter(nd::Axis(2)).into_par_iter().enumerate().for_each(|(idx, slice)| {
    ...
    if !plot_folder.exists() {
      std::fs::create_dir(plot_folder)
        .expect(&format!("could not create plotting folder {output_folder:?}"))
    }

    //Make new transform instance to save imgs
    let ws = rws::TransformBuilder::new_merging().set_plot_folder(plot_folder).build().unwrap();
